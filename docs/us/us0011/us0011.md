...
- [ ] CA9: Si la reserva se origina en una cotización, el booking hereda el rango estimado y el depósito se calcula como deposit_percentage × máx(mínimo cotizado, min_session_price); sin cotización, se mantiene el fallback 30% × min_session_price (decisión `fixs/issue-007.md`)
